...
CREATE TABLE RADARS(...) CLUSTER obs_radars_cluster (road);
CREATE TABLE OBSERVATIONS(...) CLUSTER obs_radars_cluster (road);
...
) CLUSTER obs_radars_cluster (road);
...
CREATE INDEX radars_ix ON RADARS(road, km_point);
CREATE INDEX roads_ix ON ROADS(name);
CREATE INDEX observations_ix ON OBSERVATIONS(road, km_point, direction);

CREATE INDEX vehicles_ix ON VEHICLES(nPlate);


-- result
--RESULTS AT 03/05/18
--TIME CONSUMPTION: 927481 milliseconds.
--CONSISTENT GETS: 11034403,5 blocks
